...
class ProductSeeder extends Seeder
{
    public function run()
    {
        $productTable = $this->db->table('product');
        ...
        foreach ($data as $item) {
            $existingProduct = $productTable->where('nama', $item['nama'])->get()->getRowArray();

            if ($existingProduct) {
                $productTable->where('id', $existingProduct['id'])->update($item);
            } else {
